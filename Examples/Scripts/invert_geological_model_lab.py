...
def forward_model(x, self, bc='Z'):
    ...
    k_list, H_list = np.array_split(x[:2*nl], 2)
    Tb = x[2*nl:]
...
def adjoint_model(x, self, bc='Z'):
    ...
    for j in range(i):
        ...
        dTb_local  = db[lowerBC_mask]/inv.ghost_weights[lowerBC_mask]
        dTb_global = dTb_local #np.array(0.0)
...
# modify layer voxel to include nodes beneath LAB
lab_mask = mesh.grid_coords[-1] < -50e3

for l in [17, 18, 19, 20]:
    i0, j0, k0 = np.where(layer_voxel == l)
    j0 = np.unique(j0)
    k0 = np.unique(k0)
    for j in j0:
        for k in k0:
            layer_voxel[lab_mask,j,k] = l
...
mu_Tb = np.ones(bc_size)*(1350.0+273.14)
sigma_Tb = np.ones(bc_size)*(1000.0)
Tbp = InvPrior(mu_Tb, sigma_Tb)
...
Tb_lower  = np.ones(bc_size)*bounds['Tb'][0]
k_upper   = np.ones(size)*bounds['k'][1]
H_upper   = np.ones(size)*bounds['H'][1]
Tb_upper  = np.ones(bc_size)*bounds['Tb'][1]

...
parser.add_argument('--nsim', required=True, type=int, help='Number of simulations')
...
def sensitive_priors(prior):
    """
    Vary sensitivity of priors in objective function.
    Prior is perturbed by randomly sampling within a normal distribution.

    Arguments
    ---------
     prior : objective variable

    Returns:
     new_prior : perturbed objective variable
...
mesh.save_mesh_to_hdf5(args.echo + '_mesh.h5')
...
for i in range(args.nsim):
    ...
    kpi = sensitive_priors(kp)
    Hpi = sensitive_priors(Hp)
    Tbpi = sensitive_priors(Tbp)
    inv.add_prior(k=kpi, H=Hpi, Tb=Tbpi)
